...
def create_production_orders_for_sales_order(
    *,
    ...
    for line in sales_order.lines.select_related(
        "product_model",
        "product_variant",
        "color",
        ...
        for (
            ...
            secondary_color,
            required_qty,
        ) in _iter_line_requirements(line=line):
            ...
            for _ in range(quantity_to_produce):
                created_orders.append(
                    create_production_order(
                        ...
                        urgent=False,
                        etsy=False,
                        comment=f"Sales order #{line.customer_order_id}, line #{line.id}",
                        created_by=created_by,
                        orders_url=orders_url,
...
def sync_sales_order_line_production(line: SalesOrderLine) -> None:
    ...
    _sync_sales_order_status(line.customer_order)


def _save_bundle_component_colors(*, line: SalesOrderLine, line_data: dict[str, object]) -> None:
    ...
    for component_data in component_colors:
        ...
        component_line = SalesOrderLineComponentSelection.objects.create(
            order_line=line,
...
def _save_bundle_preset_components(*, line: SalesOrderLine) -> None:
...
def _validate_line_material_colors(*, line: SalesOrderLine) -> None:
...
def _validate_component_material_colors(component_line: SalesOrderLineComponentSelection) -> None:
...
def _iter_line_requirements(*, line: SalesOrderLine) -> list[Requirement]:
...
def _resolve_quantity_to_produce(
    *,
    ...
    if line.production_mode == SalesOrderLine.ProductionMode.FORCE:
        return required_qty
    if line.production_mode == SalesOrderLine.ProductionMode.MANUAL:
        return 0
