...
def test_import_legacy_defaults_to_dry_run_mode():
    output = StringIO()
    with patch("apps.orders.management.commands.import_legacy.run_import_legacy") as run_import:
...
def test_import_legacy_final_mode():
    output = StringIO()
    with patch(
        "apps.orders.management.commands.import_legacy.run_final_import_and_verify"
    ) as run_final_import:
