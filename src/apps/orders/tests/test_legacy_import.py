...
from apps.orders.legacy_import import run_final_import_and_verify, run_import_legacy
...
from apps.orders.tests.conftest import OrderFactory
...
def test_verify_mode_returns_finished_balance_by_warehouse():
    ...
    stock_record = StockRecord.objects.create(
        warehouse=warehouse,
        product_model=ProductModelFactory(is_bundle=False),
        color=ColorFactory(),
        quantity=5,
    )
