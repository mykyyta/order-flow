...
| Змінна | Тип | Опис |
|--------|-----|------|
| `show_page_header` | bool | Показувати in-content заголовок (`h1`) і actions (default: `True`). Для chips-сторінок став `False`. |
...
| Змінна | Тип | Опис |
|--------|-----|------|
| `back_url` | str | URL для лінка "Назад" (рендериться над карткою). |
| `back_label` | str | Текст лінка (default: "Назад"). |
...
| Змінна | Тип | Опис |
|--------|-----|------|
| `back_url` | str | URL для лінка "Назад". |
| `back_label` | str | Текст лінка (default: "До активних"). |

**Блоки:**
| Блок | Призначення |
|------|-------------|
| `archive_items` | Рендер елементів архіву (chips). |
| `archive_body` | Повна розмітка архіву (коли потрібна таблиця/лист). |
